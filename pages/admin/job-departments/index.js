...
const AdminJobDepartmentPage = ({user, children}) => {
    ...
    const handleDeleteMany = function (id = null) {
        ...
        ApiCall({
            router,
            path: `/api/v1/admin/job-departments?${query}`,
            method: "DELETE",
            token: user.token,
        }).then((res) => {
            if (res.status) {
        ...
        // },
        {
            ...
            render: (field, row, index) => {
                if (canDetail && canEdit) {
                    return <AppLink href={`/admin/job-departments/edit/${row.id}`}>{field}</AppLink>;
                } else {
                    return field
                }
            }
    ...
    const createBtn = (
        <React.Fragment>
            ...
            {canCreate ? <AdminButton href="/admin/job-departments/create">Create</AdminButton> : ''}
        </React.Fragment>
    );

    const header = (
        <PageHeader pretitle="admin" title="Department" button={createBtn} separator={true}>
            Department
        </PageHeader>
    );
...
export default AdminJobDepartmentPage;
