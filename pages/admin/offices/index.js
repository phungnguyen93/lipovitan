...
const AdminOfficesPage = ({ user, children }) => {
    ...
    //Permissions
    const canList               = checkPermission(user, 'office_list');
    const canCreate             = checkPermission(user, 'office_add');
    const canEdit               = checkPermission(user, 'office_edit');
    const canDetail             = checkPermission(user, 'office_detail');
    const canDelete             = checkPermission(user, 'office_delete');
    ...
    const fetchList = function(pagination = null, filters = null, sorter = null) {
        ...
        ApiCall({
            router,
            path: `/api/v1/admin/offices?page=${page}&limit=${limit}&orderBy=${orderBy}&order=${order}&${filtera}`,
            token: user.token,
        }).then((res) => {
    ...
    const handleDeleteMany = function(id = null) {
        ...
        ApiCall({
            router,
            path: `/api/v1/admin/offices?${query}`,
            method: "DELETE",
            token: user.token,
        }).then((res) => {
            if (res.status) {
    ...
    const columns = [
        {
            ...
            render: (title, row, index) => {
                if(canDetail && canEdit) {
                    return <AppLink href={`/admin/offices/edit/${row.id}`}>{title}</AppLink>;
                } else {
                    return title
                }
            }
    ...
    const createBtn = (
        <React.Fragment>
            ...
            {canCreate ? <AdminButton href="/admin/offices/create">Create</AdminButton> : ''}
        </React.Fragment>
    );

    const header = (
        <PageHeader pretitle="admin" title="Office" button={createBtn} separator={true}>
            Office
        </PageHeader>
    );
