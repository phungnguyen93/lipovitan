...
const AdminJobCreatePage = ({user}) => {
        ...
        },
        {
            ...
            render: (text, record) =>
                content.en.dataSource.length >= 1 ? (
                    // <Popconfirm title="Sure to delete?" onConfirm={() => handleDelete(record.key, 'en')}>
    ...
    const header = (
        <PageHeader pretitle="Job" title="Create Job" button={<BackButton/>} separator={true}>
        </PageHeader>
    );
