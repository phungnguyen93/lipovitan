...
import { Table, Divider, Select, Image, Popconfirm } from "antd";
...
const AdminGtStoresPage = ({ user, children }) => {
    ...
    //Permissions
    const canList               = checkPermission(user, 'gt_store_list');
    const canCreate             = checkPermission(user, 'gt_store_add');
    const canEdit               = checkPermission(user, 'gt_store_edit');
    const canDetail             = checkPermission(user, 'gt_store_detail');
    const canDelete             = checkPermission(user, 'gt_store_delete');
    ...
    const fetchList = function(pagination = null, filters = null, sorter = null) {
        ...
        ApiCall({
            router,
            path: `/api/v1/admin/gt-stores?page=${page}&limit=${limit}&orderBy=${orderBy}&order=${order}&${filtera}`,
            token: user.token,
        }).then((res) => {
    ...
    const handleDeleteMany = function(id = null) {
        ...
        ApiCall({
            router,
            path: `/api/v1/admin/gt-stores?${query}`,
            method: "DELETE",
            token: user.token,
        }).then((res) => {
            if (res.status) {
        ...
        },
        {
            ...
            render: (title, row, index) => {
                if(canDetail && canEdit) {
                    return <AppLink href={`/admin/gt-stores/edit/${row.id}`}>{title}</AppLink>;
                } else {
                    return title
                }
            }
            ...
            render: (text, row, record) => {
                    ...
                    return (
                        <>
                            {
                                canEdit &&
                                <AppLink href={`/admin/gt-stores/edit/${row.id}`}>
                                    <AdminButton type={ButtonType.WARNING} size={ButtonSize.SMALL}>
    ...
    const createBtn = (
        <React.Fragment>
            ...
            {canCreate ? <AdminButton href="/admin/gt-stores/create">Create</AdminButton> : ''}
        </React.Fragment>
    );

    const header = (
        <PageHeader pretitle="admin" title="GT Channels" button={createBtn} separator={true}>
            GT Channels
        </PageHeader>
    );
...
export default AdminGtStoresPage;
