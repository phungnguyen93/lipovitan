...
const AdminPostsPage = ({ user, children }) => {
        ...
        },
        {
            ...
            render: (title, row, index) => {
                if(canDetail && canEdit) {
                    return <AppLink href={`/admin/posts/edit/${row.id}`}>{title}</AppLink>;
                } else {
                    return title
                }
            }
    ...
    const createBtn = (
        <React.Fragment>
            ...
            {canCreate ? <AdminButton href="/admin/posts/create">Create</AdminButton> : ''}
        </React.Fragment>
    );
