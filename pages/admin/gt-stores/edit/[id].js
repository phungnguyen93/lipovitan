...
const AdminGtStoresEditPage = ({user}) => {
    ...
    const fetchDetail = function () {
        ApiCall({
            router,
            path: `/api/v1/admin/gt-stores/${id}`,
            token: user.token,
        }).then((res) => {
            if (res.status) {
    ...
    const header = (
        <PageHeader pretitle="GT Channels" title="Update Channel" button={<BackButton/>} separator={true}>
        </PageHeader>
    );
...
export default AdminGtStoresEditPage;
