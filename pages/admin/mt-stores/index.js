...
const AdminMtStoresPage = ({ user, children }) => {
    ...
    //Permissions
    const canList               = checkPermission(user, 'mt_store_list');
    const canCreate             = checkPermission(user, 'mt_store_add');
    const canEdit               = checkPermission(user, 'mt_store_edit');
    const canDetail             = checkPermission(user, 'mt_store_detail');
    const canDelete             = checkPermission(user, 'mt_store_delete');
    ...
    const fetchList = function(pagination = null, filters = null, sorter = null) {
        ...
        ApiCall({
            router,
            path: `/api/v1/admin/mt-stores?page=${page}&limit=${limit}&orderBy=${orderBy}&order=${order}&${filtera}`,
            token: user.token,
        }).then((res) => {
    ...
    const handleDeleteMany = function(id = null) {
        ...
        ApiCall({
            router,
            path: `/api/v1/admin/mt-stores?${query}`,
            method: "DELETE",
            token: user.token,
        }).then((res) => {
            if (res.status) {
            ...
            render: (text, row, record) => {
                    ...
                    return (
                        <>
                            {
                                canEdit &&
                                <AppLink href={`/admin/mt-stores/edit/${row.id}`}>
    ...
    const createBtn = (
        <React.Fragment>
            ...
            {canCreate ? <AdminButton href="/admin/mt-stores/create">Create</AdminButton> : ''}
        </React.Fragment>
    );

    const header = (
        <PageHeader pretitle="admin" title="Modern Channels" button={createBtn} separator={true}>
            Modern Channels
        </PageHeader>
    );
...
export default AdminMtStoresPage;
